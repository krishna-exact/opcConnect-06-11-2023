        ...
        async.series([
            ...
            function (callback) {
                ...
                tags.forEach(function (tag) {
                    try {
                        let nodeId;
                        let attributeId = AttributeIds.Value; // Default attributeId
                
                        if (tag && tag["address"] !== undefined && tag["address"] !== null) {
                            // Handle tag with "address" property
                            tag = tag["address"];
                            nodeId = `ns=${process.env.OPC_SERVER_CLSID};${process.env.OPC_SERVER_PREFERENCE}=${tag}`;
                        } else if (tag && typeof tag["dataTagId"] === "string") {
                            // Handle tag with "dataTagId" property as a string
                            tag = tag["dataTagId"];
                            if (process.env.OPC_SERVER_PREFERENCE === "s") {
                                nodeId = `ns=${process.env.OPC_SERVER_CLSID};${process.env.OPC_SERVER_PREFERENCE}=${process.env.OPC_SERVER_ADDRESS_PREFIX}${tag}${process.env.OPC_SERVER_ADDRESS_SUFFIX}`;
                            } else if (process.env.OPC_SERVER_PREFERENCE === "i") {
                                console.log("Skipping node tag when OPC_SERVER_PREFERENCE is set to 'i'.");
                                return; // Skip processing this tag
                            } else {
                                console.log("Invalid OPC_SERVER_PREFERENCE value. Skipping this tag.");
                                return;
                            }
                        } else {
                        ...
                        console.log("attributeId::::", AttributeIds.Value);
                        console.log("nodeId:::::", "ns=" + process.env.OPC_SERVER_CLSID + ";" + process.env.OPC_SERVER_PREFERENCE + "=" + tag);
                ...
                // Continue with your monitoring logic using the itemsToMonitor array
