...
angular.module('desktopApp', [])
    .controller('indexController', function ($scope, $http, $interval) {
        ...
        $scope.stop = function (configId) {
            console.log("Calling stop wala stop::::::",configId)
            stop(configId)
            console.log("Calling after stop::::::",configId)
        ...
        $scope.restart = function (configId) {
            console.log("restart wala:::",configId)
        ...
        $scope.stopAll = function (configId) {
        ...
        $scope.copyLogs = function(configId) {
            // Select the text content of <pre> tag with the specific id
            var logsContent = angular.element(document.getElementById(configId)).text();
...
// Define a controller within the module
    $scope.copyToClipboard = function (configId) {
        ...
        var dynamicPathElement = angular.element(document.getElementById("dynamic-path"));
    ...
    $scope.fetchDynamicPath = function() {
        var username = window.process && window.process.env && window.process.env.USERPROFILE ? window.process.env.USERPROFILE.split('\\')[2] : "";
    ...
    $scope.toggleCertificatePathVisibility = function() {
        ...
        $interval(function () {
            ...
            $http.get(userApiUrl, { responseType: 'arraybuffer' })  // Replace with the actual server URL
                .then(function (response) {
                    // Calculate request time in milliseconds
                    var endTime = new Date().getTime();
                    var requestTime = endTime - startTime; // in milliseconds
        
                    // Calculate download speed in Mbps and Kbps
                    var downloadSpeedMbps = ((response.data.byteLength / (requestTime / 1000)) * 8 / 1000000)*10;
                    var downloadSpeedKbps = (downloadSpeedMbps * 1000)*10;

                    console.log("downloadSpeedMbps",downloadSpeedMbps);
                    console.log("downloadSpeedKbps",downloadSpeedKbps);
        
        //             // document.getElementById('network-status').textContent = 'Connected';
                    // document.getElementById('network-speed-mbps').textContent = downloadSpeedMbps.toFixed(2) + ' Mbps';
        //             // document.getElementById('network-speed-kbps').textContent = downloadSpeedKbps.toFixed(2) + ' Kbps';

        var networkSpeedElement = document.getElementById('network-speed-mbps');

        var divElement = document.getElementById('network-speed-mbps'); // Get the div element
        // id="network-speed-mbps"
                    // Check if download speed is less than 2 Mbps
